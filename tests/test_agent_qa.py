...
def _chat(api, message: str, **kwargs: Any) -> dict[str, Any]:
    payload = {
        "message": message,
        "user_id": kwargs.get("user_id", "123"),
        "language": kwargs.get("language", "bg"),
    }
    if "fault" in kwargs:
...
def test_english_language_reply(api):
    body = _chat(api, "Where is my order 456?", language="en")
    assert body["language"] == "en"
    assert _tool_names(body) == ["get_order"]
    reply = body["reply"].lower()
    assert "order 456" in reply
